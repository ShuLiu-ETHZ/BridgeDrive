...
from lead.training import training_utils_bridgedrive
...
class Trainer:
    def __init__(self):
        self.config = training_utils_bridgedrive.initialize_config()

        self.ssd_cache = training_utils_bridgedrive.initialize_training_session_cache(self.config)
        self.num_worker = training_utils_bridgedrive.initialize_torch(self.config)
        self.model_wrapper, self.cur_epoch = training_utils_bridgedrive.initialize_model(
            self.config
        )
        self.model = self.model_wrapper
        if isinstance(self.model_wrapper, torch.nn.parallel.DistributedDataParallel):
            self.model = self.model_wrapper.module
        self.dataloader, self.sampler = training_utils_bridgedrive.initialize_dataloader(
        ...
        self.optimizer, self.scheduler, self.scaler, self.gradient_steps_skipped = (
            training_utils_bridgedrive.initialize_optimizer(
        ...
        training_utils_bridgedrive.save_config(self.config, self.config.rank)
...
def main():
    training_utils_bridgedrive.increase_limit_file_descriptors()
    Trainer().train_loop()


if __name__ == "__main__":
    training_utils_bridgedrive.set_start_method()
    main()
