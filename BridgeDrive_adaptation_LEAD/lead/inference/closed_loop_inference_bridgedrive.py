...
# BridgeDrive
from lead.inference.open_loop_inference_bridgedrive import OpenLoopInference, OpenLoopPrediction
from lead.tfv6.tfv6_bridgedrive import Prediction
...
class ClosedLoopInference(OpenLoopInference):
    ...
    def execute_route_and_target_speed(
        ...
        """
        if self.config_closed_loop.diffusion_speed:
            tmp_pred_route = pred_checkpoints[0].data.cpu().numpy()
            pred_checkpoints = tmp_pred_route[:10,:]
            speed = float(speed[0].data.cpu().numpy())
            pred_target_speed = float(tmp_pred_route[10,0])
            print("use diffusion speed")
        else:
