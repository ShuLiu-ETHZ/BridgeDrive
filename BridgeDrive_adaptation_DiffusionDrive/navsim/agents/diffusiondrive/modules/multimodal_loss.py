...
class LossComputer(nn.Module):
    ...
    def forward(self, poses_reg, targets, poses_cls=None, plan_anchor=None):
        """
        pred_traj: (bs, 20, 8, 3)
        pred_cls: (bs, 20)
        ...
        if self.cls_loss_weight and num_mode > 1:
...
class DDBMLossComputer(nn.Module):
    def __init__(self,config: dict):
        self._config = config
        super(DDBMLossComputer, self).__init__()
        # self.focal_loss = FocalLoss(use_sigmoid=True, gamma=2.0, alpha=0.25, reduction='mean', loss_weight=1.0, activated=False)
        self.cls_loss_weight = config.trajectory_cls_weight
        self.reg_loss_weight = config.trajectory_reg_weight
    def forward(self, poses_reg, poses_cls, targets, plan_anchor, best_reg):
        """
        pred_traj: (bs, 20, 8, 3)
        pred_cls: (bs, 20)
        plan_anchor: (bs,20, 8, 2)
        ...
        """
        bs, num_mode, ts, d = poses_reg.shape
        num_mode = poses_cls.shape[1]
        target_traj = targets["trajectory"]


        dist = torch.linalg.norm(target_traj.unsqueeze(1)[...,:plan_anchor.shape[-2],:2] - plan_anchor, dim=-1)
        dist = dist.mean(dim=-1)
        mode_idx = torch.argmin(dist, dim=-1)
        cls_target = mode_idx
        ...
        # Calculate regression loss
        reg_loss = self.reg_loss_weight * F.l1_loss(best_reg, target_traj) # remove slicing to align shape
