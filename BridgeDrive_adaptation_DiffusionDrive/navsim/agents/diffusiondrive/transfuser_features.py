...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        status_list = []
        for s in self._config.ego_status_info:
            if s == "driving_command":
                status_list.append(
                ...
                ego_velocity_2d_ = agent_input.ego_statuses[-1].ego_velocity
                speed_ = (ego_velocity_2d_**2).sum(-1) ** 0.5
                status_list.append(
                    torch.tensor([speed_], dtype=torch.float32)
                )
            elif s == "ego_acceleration":
                status_list.append(
                    torch.tensor(agent_input.ego_statuses[-1].ego_acceleration, dtype=torch.float32)
                )
            else:
                raise ValueError(f"{s} cannot be recognized as a valid EgoStatus item!")
