...
class TransfuserConfig:
    ...
    plan_anchor_path: str = "/data/workspace/shuliu/BridgeDrive_adaptation_DiffusionDrive/kmeans_navsim_traj_20.npy"
    # head: str = "TrajectoryTruncDiffuHead"
    # head: str = "TrajectoryFullDiffuHead"
    head:str = "DDBMTrajectoryHead"
    use_scalar_norm:bool = False
    beta_d:float = 2.0

    latent: bool = False
    latent_rad_thresh: float = 4 * np.pi / 9

    #ego_status_info = ("driving_command", "ego_speed") #("driving_command", "ego_velocity") # ("driving_command", "ego_velocity", "ego_acceleration")
    ego_status_info = ("driving_command", "ego_velocity", "ego_acceleration")
    ego_status_dim: int = sum(STATUS2DIM[s] for s in ego_status_info)
    ...
    bev_semantic_resolu_height = lidar_resolution_height // 2
    bev_semantic_resolu_width = lidar_resolution_width
